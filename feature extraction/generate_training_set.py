...
def pack_data(args, name_list, read_feature_list, read_label_list, save_path):
    ...
    for name in name_list:
        ...
        for label_name in read_label_list:
            name = os.path.basename(name)
            path = os.path.join(args.data_path, label_name, name)
...
def parse_args():
    ...
    parser.add_argument("--save_path",  default = os.path.join(dir_path,'training_set'), type=str, help = 'path to save training set')
