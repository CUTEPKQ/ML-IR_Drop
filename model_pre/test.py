...
def test():
    ...
    if arg_dict['cpu']:
        ...
        logger.info('using cpu for testining')
    ...
    for feature, label, instance_count_path, instance_IR_drop_path, instance_name_path in dataset:
        ...
        if not arg.final_test:
            for metric, metric_func in metrics.items():
                result = metric_func(instance_IR_drop, pred_instance_irdrop)

                if result > max_mae:
                    max_mae = result
            
                    show = [instance_IR_drop,pred_instance_irdrop]
        ...
        if arg_dict['plot']:
            ...
            fig.savefig(os.path.join(save_path, file_name + '_pred_ir_drop.png'), dpi=100)
            plt.close()

            fig = sns.heatmap(data=label.squeeze().numpy()[:,:], cmap="rainbow").get_figure()
            fig.savefig(os.path.join(save_path, file_name + '_label_ir_drop.png'), dpi=100)
            plt.close()
    ...
    if not arg.final_test:
        ...
        for metric, avg_metric in avg_metrics.items():
            ...
            # logger.info("===> Avg. {}: {:.6f}".format(metric, avg_metric / 100 )) 
        for metric, design in split_metrics.items():
            if len(design) == 1:
                continue
            for name, values in design.items():
                logger.info("===> {} {}: {:.6f}".format(name, metric, values[0] / values[1]))
                # logger.info("===> {} {}: {:.6f}".format(name, metric, values[0] / values[1]))
